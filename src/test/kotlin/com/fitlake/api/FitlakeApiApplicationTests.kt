...
class FitlakeApiApplicationTests {
	...
	fun `configuration externalizes secrets and delegates schema management to Flyway`() {
		...
		assertEquals("\${SPRING_AI_MODEL_CHAT:none}", properties.getProperty("spring.ai.model.chat"))
		assertEquals("\${OPENAI_API_KEY:none}", properties.getProperty("spring.ai.openai.api-key"))
		assertEquals(
			"\${SPRING_AI_OPENAI_BASE_URL:https://api.openai.com}",
			properties.getProperty("spring.ai.openai.base-url"),
		)
		assertEquals(
			"\${SPRING_AI_OPENAI_CHAT_MODEL:}",
			properties.getProperty("spring.ai.openai.chat.model"),
		)
